...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Usage: python script.py <s3_bucket_name>  <local_folder_path> <cluster_name> <run_name>")
        sys.exit(1)

    # Get parameters from command line arguments
    s3_bucket_name = sys.argv[1]          # Your S3 bucket name
    local_folder_path = sys.argv[2]       # Path to your local folder
    cluster_name = sys.argv[3]            # Cluster name
    run_name = sys.argv[4]                # Run name
    ...
    upload_directory_to_s3(local_folder_path, s3_bucket_name, s3_directory_name)
